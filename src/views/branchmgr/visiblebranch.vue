...
import {getRoleQueryPost,getbranchtreeinfo,getgetbranchListProp} from '@/api/sysmgr'
export default {
  mixins: [mixin,wares],
  name:'visiblebranch',
   data() {
  ...
  mounted () {
    ...
    getgetbranchListProp(param).then((res)=>{
        this.tableData=res.data.data.list;
    })
  },
  activated () {
    this.$parent.linksw(this.linklist)
    this.form=this.$route.query.number?this.$route.query:this.form;
  },
  methods: {
    successBranch(){
    ...
    infoDao(){
       ...
       if(target=='关闭'){
         ...
         this.$confirm('是否保存对落地人员信息的修改?','提示').then(()=>{
