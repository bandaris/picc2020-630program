...
import sumBlackImportRouter from '@/router/modules/blackmgr/sumBlackImport'
...
export default {
  ...
  methods: {
    ...
    getRedirect() {//获取路由子列表
      let rl = sumBlackImportRouter.children;
      if (!rl) return;
    ...
    addnav(res) {//向navList中添加项每次跳转时首先判断调用
      ...
      let index = this.$refs.navlist.navList.push(res);//push方法会返回下标值
