<template>
 <div :class="config.isRadio?'treeBox radio':'treeBox'" ref="treeBox">
     ...
     @check="checkUpdata"
     @node-click="getUserInfo">
        <span class="custom-tree-node" slot-scope="{ node, data }">
            <img src="./../assets/image/treeIcon.png" alt="" v-if="config.isShowIcon">
            <span :class="data.deptcode==activeitem?'activeitemfocus':''">{{ data.deptname }}</span>
        </span>
    </el-tree>
 </div>
</template>
...
export default {
 ...
 mounted() {
   eventBus.$on('tree-reloadnode',edata=>this.reloadNode(edata))
 ...
 methods: {
    ...
    handleClick(data,checked,node){//实现单选
        if(checked === true&&this.config.isRadio) {
          this.$refs.tree.setCheckedKeys([data.deptcode]);
