...
instance.interceptors.request.use(
  config => {
   ...
   //追加当前登录坐席信息,如果无登录信息默认userid为unknown
   let uid=lu.userinfo.userid?lu.userinfo.userid:'unknown'
   let ucd=lu.userinfo.nusercode?lu.userinfo.nusercode:'unknown'
   if(config.data && Object.prototype.toString.call(config.data)=='[object Object]' ){
    if(!config.data.attr)
      config.data.attr={}
    config.data.attr.userid=uid
    config.data.attr.nusercode=ucd
   }
...
class request {
  ...
  static async get(url,params) {
    console.log(params);
    return await instance.get(url, {params}); //eslint-disable-line
  }
  static async post(url, params) {
    ...
    tp=tp ? tp : params;
    return await instance.post(url, tp); //eslint-disable-line
  }
}
